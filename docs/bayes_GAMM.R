...
post_surv <- function(x, status = "status", time = "time", surv_form = NULL, prior = NULL, ...) {
  ...
  if(!is.null(surv_form)) {
    ...
    form <- as.formula( "status~1+offset(log_dtime)+s(time)")
  }
  
  #prepare long format dataset
  x$status <- unlist(x[status])
  x$time <- unlist(x[time])
  long_x <- gen_stan_dat(x, status = status, time = time)
