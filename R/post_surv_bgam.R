...
post_surv <- function(x, status = "status", time = "time", surv_form = NULL, prior = NULL, form = NULL,  ...) {
  ...
  if(!is.null(surv_form)) {
    input.formula <- as.formula( paste0(c( "status~1+offset(log_dtime)+s(time)+", paste(surv_form, collapse = "+")), collapse = ""))
  } else{
    if(!is.null(form) ) {
      input.formula <- form
    } else {
      input.formula <- as.formula( "status~1+offset(log_dtime)+s(time)")
    }
  }
